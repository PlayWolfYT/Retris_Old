...
public class GameBoard extends GameBoardObject {
    ...
    public void newPiece() {
        ...
        currentX = 0;
        currentY = BOARD_HEIGHT / 2 + currentPiece.getMinY();


        if (!tryMove(currentPiece, currentX + 1, currentY)) {
    ...
    public void removeFullLines() {
        ...
        for(int i = BOARD_WIDTH; i >= 0; --i) {
            ...
            for(int j = 0; j < BOARD_HEIGHT; j++) {
                if(getShapeAt(i, j) == Retrominos.NoShape) {
                    lineIsFull = false;
                    break;
                }
            }
            ...
            if(lineIsFull) {
                ...
                for(int x = i; x >= 1; --x) {
                    for(int y = 0; y < BOARD_HEIGHT; y++) {
                        board[y * BOARD_WIDTH + x] = getShapeAt(x - 1, y);
                    }
                }
            }
        }
    ...
    public void dropDown() {
        ...
        while (newX > 0) {
            if (!tryMove(currentPiece, newX + 1, currentY))
                break;
